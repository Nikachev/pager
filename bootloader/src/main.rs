...
fn main() -> ! {
    ...
    let chosen = if control.trial_slot != boot_control::NONE {
        ...
        unsafe { boot_control::write(boot_control::seal(control.trial(candidate.0, candidate.1)), control_page) };
        candidate.0
    } else {
        required_slot(confirmed.or_else(|| newest_slot(&slots)))
    };
...
fn read_slot(slot: u32) -> Option<(u32, u32)> {
    ...
    let image = unsafe {
        core::slice::from_raw_parts(slot_image_start(slot) as *const u8, manifest.image_len as usize)
    };
    manifest.verifies(image, SLOT_IMAGE_CAPACITY).then_some((slot, manifest.version))
}

fn slot_matches(slots: &[Option<(u32, u32)>; 2], slot: u32, version: u32) -> Option<u32> {
    slots.iter().flatten().find(|(s, v)| *s == slot && *v == version).map(|(s, _)| *s)
}

fn newest_slot(slots: &[Option<(u32, u32)>; 2]) -> Option<u32> {
    slots.iter().flatten().max_by_key(|(_, version)| *version).map(|(slot, _)| *slot)
}

fn newer_than(slots: &[Option<(u32, u32)>; 2], version: u32) -> Option<(u32, u32)> {
    slots.iter().flatten().filter(|(_, candidate)| *candidate > version).max_by_key(|(_, candidate)| *candidate).copied()
}

fn slot_manifest_start(slot: u32) -> u32 {
    if slot == manifest::SLOT_A { SLOT_A_MANIFEST } else { SLOT_B_MANIFEST }
}

fn slot_image_start(slot: u32) -> u32 {
    if slot == manifest::SLOT_A { SLOT_A_IMAGE } else { SLOT_B_IMAGE }
}
